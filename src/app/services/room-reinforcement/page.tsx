...
const faqs = [
  ...
  {
    q: "צריך אישור פקע״ר?",
    a: "תלוי בהיקף. חיזוק קונסטרוקטיבי או החלפת דלת הדף — לרוב כן. החלפות קוסמטיות — לא. בודקים לפני שמתחילים ומטפלים בהיתרים הנדרשים. דרישות הרשויות המקומיות משתנות — חלק מהעיריות מחמירות יותר מאחרות, ולפעמים נדרש גם אישור של מחלקת ההנדסה המקומית על שינויים קונסטרוקטיביים. [טעון אימות מקצועי על דרישות ספציפיות לפי עירייה]",
  },
  {
    q: "איך עובד אוורור וסינון במיגון חדר קיים?",
    a: "מיגון חדר קיים לא כולל באופן אוטומטי מערכת סינון NBC כמו בממ״ד תקני. במקרים מסוימים ניתן להוסיף גוף סינון ופתח אוורור מוגן, אבל זו תוספת מעל למיגון הבסיסי ולא סטנדרט. מה שכן נעשה — שיפור אטימה (סגירת פתחים, מסגרות חלון חזקות), ואם יש פתח אוורור קיים — החלפתו במכסה חזק עם אפשרות סגירה. לציין — חדר ממוגן ללא סינון אינו מספק את אותה רמת הגנה מפני חומרים כימיים כמו ממ״ד תקני. למי שרוצה הגנה כימית מלאה — הפתרון הנכון הוא ממ״ד.",
  },
  {
    q: "יתרונות מול ממ״ד?",
  ...
  },
  {
    ...
    a: "דלת חיזוק היא דלת פלדה משוריינת עם נעילה חזקה ומסגרת מחוזקת — מיועדת בעיקר נגד פריצה ונותנת גם שיפור מסוים של התנגדות להדף. דלת הדף היא דלת עם אישור פקע״ר רשמי, תוכננה לעמוד בגל הדף מפיצוץ, עם אטימה לאוויר ולחומרים כימיים. דלת הדף יקרה משמעותית (15,000–30,000 ₪ לעומת 4,000–10,000 ₪ לדלת חיזוק) [טעון אימות מקצועי]. במיגון חדר אנחנו מתאימים את הדלת להיקף המיגון — דלת חיזוק מספקת בהחלפה קוסמטית, דלת הדף נכונה בחיזוק קונסטרוקטיבי כולל.",
  },
  {
    q: "מה אנחנו לא ממליצים למגן?",
    a: "חדר ללא קיר חיצוני חזק (למשל חדר עם קיר גבס בלבד מצד חוץ) — שם החיזוק לא מעשי. חדר עם חלונות גדולים בקיר חיצוני — אפשר לשדרג חלונות אבל ההגנה מוגבלת. קומה עליונה ללא חיזוק תקרה מעל — תקרה רגילה לא מגנה על החדר מלמעלה. דירות שכירות (בלי הסכמת בעל הבית על שינויים קונסטרוקטיביים) — לא חוקי ולא משתלם. במקרים אלה — נייעץ לחשוב על ממ״ד חיצוני, מיגונית בחצר, או מעבר לדירה עם ממ״ד.",
  },
  {
        ...
        <Reveal>
          <div className="max-w-4xl mx-auto">
            ...
            <p className="text-[var(--color-ink)] leading-relaxed mb-4">
              מיגון חדר קיים הוא פתרון טוב כאשר ממ״ד חדש לא אפשרי. אבל יש מצבים שבהם מיגון לא מספיק — ולכן חשוב לדעת מראש מתי להעדיף ממ״ד תקני:
            </p>
