...
function introTemplate(idx: number, city: string, svcName: string, blurb: string): string[] {
  const templates = [
    [
      `${svcName} ב${city} — זה התחום של חברת התחדשות בינוי ויזמות. ${blurb} בכל פרויקט שאנחנו מבצעים ב${city}, אנחנו הקבלנים המבצעים בפועל — לא מתווכים, לא מעבירים את העבודה לקבלני משנה אנונימיים. הלקוח חותם חוזה אחד מול חברה אחת, ומקבל אחריות מלאה גם על התכנון וגם על הביצוע.`,
    ...
    [
      ...
      `כחברת בינוי ויזמות שפועלת בפריסה ארצית, אנחנו מכירים את הדרישות הספציפיות ל${city} — ועדות מקומיות, טופוגרפיה, רמות איום — ומתאימים את התכנון ואת המפרט לכל פרויקט בנפרד. אין הצעה גנרית. יש הצעה אחת — שלכם. עם מחיר סגור, לוח זמנים מוגדר, ואחריות קבלן לפי חוק על כל העבודה.`,
    ],
    [
      `לפני שמתחילים פרויקט ${svcName} ב${city}, חשוב להבין מה בדיוק כולל השירות ומה הוא לא. ${blurb}`,
      `בהתחדשות אנחנו מקפידים על שקיפות מלאה. הצוות ב${city} כולל אדריכל שמכיר את הוועדה המקומית, קונסטרוקטור שמחשב את ההעמסה והחיזוקים, וקבלני ביצוע שמבצעים את העבודה בפועל. אנחנו לא עובדים בקיצורי דרך — כי ממ״ד או מיגון שלא בוצעו לפי התקן פשוט לא יאושרו על ידי פקע״ר, וגם אם יאושרו, לא ימלאו את תפקידם במקרה חירום. אצלנו הכל לפי הספר.`,
    ],
    [
  ...
  const faqs = [
    {
      ...
      a: `העלות של ${svc.name} ב${area.name} משתנה לפי סוג הנכס, גודל, מיקום מדויק ודרישות פקע״ר באזור. בבניית ממ״ד טיפוסי טווחי המחיר הכלליים נעים בין 160,000 ל-220,000 ₪ + מע״מ, עם תוספת אפשרית באזורי קו קדמי. מיגון חדר קיים ומיגוניות זולים יותר. ההצעה הסופית ניתנת אחרי סיור ומדידה.`,
    },
    {
  ...
  return (
    <>
      <PageHero
        ...
        subtitle={`חברת התחדשות בינוי ויזמות — ${svc.shortName} ב${area.name} עם ליווי הנדסי מלא, אישורי פיקוד העורף ואחריות קבלן.`}
        crumbs={[
        ...
        <div className="grid md:grid-cols-2 gap-5">
          {[
            ...
            },
            {
              ...
              d: "הקבלנים שלנו מבצעים את העבודה בפועל. פיקוח יומיומי. אין העברה לקבלני משנה אנונימיים.",
            },
            {
              t: "מסירה ואחריות",
