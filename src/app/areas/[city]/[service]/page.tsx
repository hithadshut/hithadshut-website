...
function introTemplate(idx: number, city: string, svcName: string, blurb: string): string[] {
  const templates = [
    [
      `${svcName} ב${city} הוא התחום של חברת התחדשות בינוי ויזמות. ${blurb} בכל פרויקט שאנחנו מבצעים ב${city}, אנחנו הקבלנים המבצעים בפועל, לא מתווכים, לא מעבירים את העבודה לקבלני משנה אנונימיים. הלקוח חותם חוזה אחד מול חברה אחת, ומקבל אחריות מלאה גם על התכנון וגם על הביצוע.`,
    ...
    [
      ...
      `בהתחדשות אנחנו מקפידים על שקיפות מלאה. הצוות ב${city} כולל אדריכל שמכיר את הוועדה המקומית, קונסטרוקטור שמחשב את ההעמסה והחיזוקים, וקבלני ביצוע שמבצעים את העבודה בפועל. אנחנו לא עובדים בקיצורי דרך. ממ״ד או מיגון שלא בוצעו לפי התקן פשוט לא יאושרו על ידי פקע״ר, וגם אם יאושרו, לא ימלאו את תפקידם במקרה חירום. אצלנו הכל לפי הספר.`,
    ],
    [
