    ...
    rows: [
      {
        label: "רישיון ותקנים",
        value:
          "רשום בפנקס הקבלנים של משרד הבינוי והשיכון עם סיווג 100 (בנייה) מתאים להיקף הפרויקט. אפשר לאמת באתר רשם הקבלנים הממשלתי.",
      },
      {
        label: "ביטוח עבודות",
        value:
      ...
      },
      {
        label: "אחריות משפטית",
        value:
          "אחריות לפי חוק המכר (דירות) ולפי חוק החוזים. אחריות של שנה על ליקויים גלויים ועד 7 שנים על ליקויים מבניים. אפשר לתבוע משפטית.",
      },
      {
        label: "חוזה בכתב",
        value:
  ...
  },
  {
    ...
    rows: [
      ...
      },
      {
        label: "חוזה בכתב",
        value:
          "אין חוזה, או חוזה של עמוד אחד בלי מפרט ותכניות. כל שינוי או תוספת ייקבעו 'בדרך', ויעלו הרבה יותר ממה שסוכם.",
      },
      {
        label: "חשבונית מס",
        value:
...
const redFlags = [
  ...
  },
  {
    ...
    text: "כל קבלן רשום חייב להציג מספר רישום בפנקס הקבלנים, אפשר לאמת ב-gov.il (רשם הקבלנים). מי שמתחמק מלתת את המספר, עזבו.",
  },
  {
    title: "אין רשימת ממליצים עם פרטי קשר",
    text: "קבלן רציני גאה בפרויקטים שלו ושמח שתדברו עם לקוחות קודמים. 'יש לי המלצות' בלי מספרי טלפון של לקוחות שאפשר לחייג אליהם, סימן אזהרה.",
  },
  {
...
const mustAskQuestions = [
  {
    ...
    a: "תשובה נכונה: מספר רישום ברור, סיווג 100 בקבוצה המתאימה (1 / 2 / 3) להיקף העבודה. אתם בודקים את המספר באתר gov.il בחיפוש רשם הקבלנים, מול השם והסיווג. לוקח דקה. קבלן שמתחמק, 'אני יושב בחברה של אחר', או נותן מספר שלא מופיע, עוזבים.",
  },
  {
    q: "האם תציג לי תעודת ביטוח עבודות קבלניות בתוקף?",
    a: "תשובה נכונה: כן, הנה התעודה עם שם החברה, תקופת ביטוח, סכומי כיסוי. אתם בודקים שהביטוח בתוקף לתקופת הפרויקט, שכיסוי צד ג׳ הוא לפחות 2,000,000 ₪, ושיש חבות מעבידים. 'הביטוח יסודר אחר-כך' משמעו לא מתחילים פרויקט.",
  },
  {
    q: "האם אני מקבל חוזה מפורט עם מפרט, תכניות ולוחות זמנים, ואת האישור באחריותך?",
    a: "תשובה נכונה: כן, חוזה קבלנות של 20+ עמודים, מפרט טכני, תכניות קונסטרוקציה ואדריכלות, לוח זמנים עם אבני דרך, סעיף אישור פקע״ר באחריות הקבלן, ערבות ביצוע. 'אני עושה את זה בלי חוזה, סומכים זה על זה' זו תשובה שלילית. גם אם ממליץ מכר.",
  },
];
...
const faqs = [
  ...
  },
  {
    ...
    a: "ארבעה סיכונים מרכזיים: (1) עובד נפצע באתר ללא ביטוח, התביעה מגיעה אליכם; (2) העבודה לא עומדת בתקן 4422 ולא מקבלת אישור פקע״ר, הממ״ד פסול; (3) רשויות המס יחקרו תשלומים במזומן ללא חשבונית; (4) אין ערבויות או אחריות, ליקוי שיתגלה בעוד שנה הוא על חשבונכם. לכל מקרה פרטני מומלץ ייעוץ משפטי.",
  },
  {
  ...
  },
  {
    ...
    a: "שלושה שלבים: (1) לתעד מיד את המצב: תמונות, וידאו, כל המסמכים; (2) להתייעץ עם עורך דין ועם מהנדס קונסטרוקציה שיבדוק את מה שנעשה; (3) להביא קבלן רשום שיעריך מה אפשר להציל ומה צריך לפרק. לפעמים עדיף לפרק ולהתחיל מחדש עם קבלן רשום מאשר להשקיע בתיקון עבודה לקויה.",
  },
];
    ...
    <>
      ...
      <Section>
        ...
        <p className="mt-6 text-xs text-[var(--color-muted)] text-center">
          המידע אינפורמטיבי בלבד, אינו ייעוץ משפטי. בדיקה פרטנית דרך{" "}
          <a
