...
### B-020 · תמונות פרויקטים אמיתיים
- **מה צריך**: 5-15 פרויקטים. לכל אחד 3-6 תמונות (לפני / במהלך / אחרי). מיקום (עיר ושכונה — לא כתובת מלאה). סוג ממ״ד. אתגר. פתרון. חודש סיום.
- **למה חוסם**: בלי תמונות אמיתיות — /projects = שלד ריק = no-go. בלי /projects = פגיעה ב-E-E-A-T ב-YMYL.
- **מה אעשה**: כשמתקבל — `/projects` index + `/projects/[slug]` per project + ImageObject schema + שילוב תמונות בעמודי שירות + עמודי אזור (case studies).
